...
public class AtomPubParser {
    ...
    private static RepositoryInfo parseRepositoryInfo(XmlPullParser parser) throws Exception {
        ...
        return AtomPubConverter.convertRepositoryInfo(repositoryInfoRawValues, repositoryCapabilitiesRawValues,
                aclCapabilities, changesOnType, extensions);
    }
        ...
        while (!(eventType == XmlPullParser.END_TAG && id.equals(name))) {
            ...
            case XmlPullParser.START_TAG:
                ...
                case XmlPullParser.TEXT:
                    ...
                    if ("\n".equals(value)) {
                        if (beforeEventType == XmlPullParser.START_TAG) {
                            ...
                            childrenStack.get(childrenStack.size() - 1).add(
                                    new CmisExtensionElementImpl(namespaces.pop(), names.pop(), attributes.pop(),
                                            (String) null));
                        }
                        parser.next();
                    } else {
                        if (childrenStack.size() != 0) {
                            childrenStack.get(childrenStack.size() - 1)
                                    .add(new CmisExtensionElementImpl(namespaces.pop(), names.pop(), attributes.pop(),
                                            value));
                            parser.next();
                        } else if (id.equals(parser.getName())) {
                            extensions.add(new CmisExtensionElementImpl(namespaces.pop(), names.pop(),
                                    attributes.pop(), value));
                            return extensions;
                        } else {
                            parser.next();
                            if (id.equals(parser.getName())) {
                                extensions.add(new CmisExtensionElementImpl(namespaces.pop(), names.pop(), attributes
                                        .pop(), value));
                                return extensions;
                            }
                        }
                ...
                case XmlPullParser.END_TAG:
                    ...
                    if (childrenStack.size() != 0) {
                        ...
                        extensions.add(new CmisExtensionElementImpl(namespaces.pop(), names.pop(), attributes.pop(),
                                (String) null));
                        return extensions;
                    }
                    break;
            ...
            case XmlPullParser.END_TAG:
                ...
                if (XmlPullParser.END_TAG == eventType) {
                    CmisExtensionElementImpl ext = new CmisExtensionElementImpl(namespaces.pop(), names.pop(),
                            attributes.pop(), childrenStack.pop());
                    if (childrenStack.size() == 0) {
                        extensions.add(ext);
                    } else {
